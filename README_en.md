...
> [!NOTE]
> (As of 2025-10) The MiniLLM series has completed pretraining of multiple model variants, with the smallest being only 25.8M (0.02B), capable of fluent conversation!
...
|------------------------|---------------------------|------------|
| MiniLLM2-small (26M)  | 0.5 GB                    | 2025.04.26 |
| MiniLLM2-MoE (145M)   | 1.0 GB                    | 2025.04.26 |
| MiniLLM2 (104M)       | 1.0 GB                    | 2025.04.26 |
| minillm-v1-small (26M)| 0.5 GB                    | 2024.08.28 |
| minillm-v1-moe (4×26M)| 1.0 GB                    | 2024.09.17 |
| minillm-v1 (108M)     | 1.0 GB                    | 2024.09.01 |

</details>

**Project Includes**

- Complete code for MiniLLM-LLM structure (Dense + MoE models).
...
- Reproduced (distilled/RL) DeepSeek-R1 reasoning model as MiniLLM-Reason model, with **data + models** fully open-sourced!
...
- Important update
- For compatibility needs, you can visit [🔗old repository content🔗](https://github.com/jingyaogong/minillm/tree/7da201a944a90ed49daef8a0265c959288dff83a).
- MiniLLM model parameters completely renamed, aligned with Transformers library models (unified naming).
...
Due to differences in Llama's positional encoding compared to minillm, there are differences in QK values after mapping Llama models.
MiniLLM2 series old models have been recovered through weight mapping and (fine-tuning training) QKVO linear layer calibration.
After this update, maintenance of the entire minillm-v1 series will be abandoned and removed from the repository.
```
...
- Major update since release, Release MiniLLM2 Series.
- Code almost completely refactored, using cleaner and more unified structure.
  For compatibility with old code, you can visit [🔗old repository content🔗](https://github.com/jingyaogong/minillm/tree/6e9cd28ef9b34a0a10afbdf6f59e65cb6e628efb).
- Eliminated data preprocessing steps. Unified dataset format, switched to `jsonl` format to avoid dataset download confusion.
- MiniLLM2 series shows significant improvement compared to MiniLLM-V1.
...
- MiniLLM2-DeepSeek-R1 series distilled models born!
- MiniLLM2 now has some English ability!
- Updated MiniLLM2 and third-party model performance results based on more large model leaderboard tests.
...
- Extended MiniLLM with multimodal capabilities---Vision
- Check out the twin project [minillm-v](https://github.com/jingyaogong/minillm-v) for details!
...
- Updated minillm-v1-moe model
- To avoid ambiguity, no longer using mistral_tokenizer for tokenization, completely using custom minillm_tokenizer as the tokenizer.
...
- Updated minillm-v1 (108M) model, using minillm_tokenizer, 3 pretraining rounds + 10 SFT rounds, more thorough training, stronger performance.
- Project has been deployed to ModelScope creation space, you can experience it on this website:
- [🔗ModelScope Online Experience🔗](https://www.modelscope.cn/studios/gongjy/minillm)
...
You can also directly download and use the `*.pth` files I trained from [here](https://www.modelscope.cn/models/gongjy/MiniLLM2-PyTorch/files).
...
You can choose to construct your own vocabulary table to train a "dictionary." The code can be found in `./scripts/train_tokenizer.py` (for learning reference only. It's not necessary to train one yourself unless required. MiniLLM comes with a built-in tokenizer).
...
Because MiniLLM size needs to be strictly controlled to avoid top-heavy models (embedding layer parameters taking up too high a proportion of LLM), shorter vocabulary lengths are better.
...
<table>
  ...
  <tr><td>minillm tokenizer</td><td>6,400</td><td>Custom</td></tr>
</table>

> 👉 Updated 2024-09-17: To prevent ambiguity from previous versions and control size, all MiniLLM models use minillm_tokenizer for tokenization, abandoning all mistral_tokenizer versions.

```
# Some thoughts
> Although minillm_tokenizer has a small length, its encoding/decoding efficiency is weaker than Chinese-friendly tokenizers like qwen2 and glm.
> But the minillm model chose the self-trained minillm_tokenizer as the tokenizer to maintain lightweight overall parameters, avoiding imbalance in encoding layer and computation layer proportions, preventing top-heavy models, because minillm's vocabulary size is only 6400.
> And minillm has never encountered rare word decoding failures in actual testing, with good results.
...
## Ⅷ MiniLLM Training Datasets

> [!NOTE]
> After 2025-02-05, all datasets used for final MiniLLM training are open-sourced. Therefore, you don't need to preprocess large-scale datasets yourself, avoiding repetitive data processing work.

MiniLLM Training Dataset Download: [ModelScope](https://www.modelscope.cn/datasets/gongjy/minillm_dataset/files) | [HuggingFace](https://huggingface.co/datasets/jingyaogong/minillm_dataset/tree/main)
...
* `lora_identity.jsonl` --Self-awareness dataset (e.g., Who are you? I am minillm...), recommended for lora training (can also be used for full-parameter SFT, don't be limited by the name)
...
* MiniLLM2 Series was trained on approximately 20GB of corpus in total, about 4B tokens, corresponding to the data combination training results above (cost: 💰💰💰💰💰💰💰💰, results: 😊😊😊😊😊😊)
...
* For friends with certain computing resources or those who care more about results, you can consider the former to fully reproduce MiniLLM2; for those with only single GPU or who care about quick reproduction in short time, we highly recommend the latter;
...
MiniLLM-Dense (same as [Llama3.1](https://ai.meta.com/blog/meta-llama-3-1/)) uses the Transformer Decoder-Only structure. The differences from GPT-3 are:
...
MiniLLM-MoE model structure is based on Llama3 and the MixFFN mixture-of-experts module from [Deepseek-V2/3](https://arxiv.org/pdf/2405.04434).
...
Zero model weights are saved as `full_sft_512_zero.pth` (see MiniLLM model file links below). If interested, you can download and test this model.
...
During training, MiniLLM's instruction and answer lengths are truncated at 512 to save GPU memory. Like learning to write, you start with short articles, and after learning to write 200-character essays, 800-character articles become easy.
...
Emphasis should be placed on introducing MiniLLM's white-box distillation code `train_distillation.py`. Since there is no powerful teacher model within the same MiniLLM series, the white-box distillation code is only for learning reference.
...
```
 {"conversations": [{"role": "user", "content": "What is your name?"}, {"role": "assistant", "content": "My name is minillm..."}]}
...
🤖️: I am MiniLLM, an artificial intelligence assistant developed by Jingyao Gong. I interact with users through natural language processing and algorithm training.
```
...
For now, MiniLLM firmly chooses to be in the distillation school. If RL on 0.1B models later achieves small progress, this training approach section will be updated.
...
After downloading the reward model, it needs to be placed in the **same level directory** as the minillm project. The recommended structure is:

```
project/
├── minillm/                    # MiniLLM project
│   ├── model/
│   └── ...
└── internlm2-1_8b-reward/       # Reward model (same level as minillm)
    ├── config.json
    ├── model.safetensors
    └── ...
```
...
<details>
<summary><b>Reward Mechanism Choice and MiniLLM Limitations (Click to expand)</b></summary>
...
**2. MiniLLM Limitation: Reward Sparsity Problem**

RLAIF training can be applied to both reasoning and non-reasoning models, the difference is only in format.

However, for MiniLLM with such tiny 0.1B parameters and weak abilities, on general tasks (like R1-style math datasets) it encounters serious reward sparsity (Reward Sparsity) problems:
...
To mitigate this problem, MiniLLM's implementation chose **model-based continuous reward signals**:
...
- In minillm practice, avoid directly using rule-based binary rewards + out-of-scope difficulty data (like MATH500), which easily leads to all-zero rewards;
...
MiniLLM's PPO implementation includes Actor model (generate answers) and Critic model (evaluate answer value), and complete GAE (Generalized Advantage Estimation) advantage function calculation.
...
On MiniLLM such ultra-small models, this problem is especially obvious. When solving math problems 99.99% of the time the entire group answer quality is poor, then cannot learn.
...
> Note: SPO is an experimental cutting-edge algorithm, MiniLLM's implementation is for exploratory learning. Due to extremely small model parameters, cannot fully reproduce paper's 8B model results.
...
<div align="center">
<p><i>MiniLLM2 (768dim) Training Curve</i></p>
</div>
...
> Considering multiple reports that Baidu Netdisk is slow, MiniLLM2 and later all use ModelScope/HuggingFace hosting.

#### ① Native PyTorch Models

MiniLLM2 Model Weights ([ModelScope](https://www.modelscope.cn/models/gongjy/MiniLLM2-PyTorch) | [HuggingFace](https://huggingface.co/jingyaogong/MiniLLM2-Pytorch))
...
|-----------------|--------|------------------------|------------------------|--------------------|------------------|----------------------------------------------|--------------------|
| MiniLLM2-small | 26M    | `pretrain_512.pth`     | `full_sft_512.pth`     | `rlhf_512.pth`     | `reason_512.pth` | `xxpo_512.pth` | `lora_xxx_512.pth` |
| MiniLLM2-MoE   | 145M   | `pretrain_640_moe.pth` | `full_sft_640_moe.pth` | `rlhf_640_moe.pth` | -                | -                                            | -                  |
| MiniLLM2       | 104M   | `pretrain_768.pth`     | `full_sft_768.pth`     | `rlhf_768.pth`     | `reason_768.pth` | `xxpo_768.pth` | `lora_xxx_768.pth` |

</details>

#### ② Transformers Models

MiniLLM Series ([ModelScope](https://www.modelscope.cn/collections/MiniLLM-b72f4cfeb74b47)
| [HuggingFace](https://huggingface.co/collections/jingyaogong/minillm-66caf8d999f5c7fa64f399e5))
...
Test based on `full_sft` and `rlhf` models of `MiniLLM2 series`, with fixed random seeds.

[A] MiniLLM2：full_sft_640<br/>
[B] MiniLLM2：rlhf_640
...
The difference between DPO and online PPO is that reject and chosen are both prepared offline, and necessarily have large distribution differences from minillm model outputs.
...
Based on `MiniLLM2 (768dim)` with same data and training steps subjective comparison, test random seed fixed at 2026, for reference:

[A] MiniLLM-Reason-0.1B (Distillation)<br/>
[B] MiniLLM-Reason-0.1B (PPO)<br/>
[C] MiniLLM-Reason-0.1B (GRPO)
...
[A] [MiniLLM2 (0.1B)](https://www.modelscope.cn/models/gongjy/MiniLLM2-PyTorch)<br/>
[B] [MiniLLM2-MoE (0.15B)](https://www.modelscope.cn/models/gongjy/MiniLLM2-PyTorch)<br/>
[C] [MiniLLM2-Small (0.02B)](https://www.modelscope.cn/models/gongjy/MiniLLM2-PyTorch)<br/>
[D] [minillm-v1-small(0.02B)](https://pan.baidu.com/s/1_COe0FQRDmeapSsvArahCA?pwd=6666)<br/>
[E] [minillm-v1-moe(0.1B)](https://pan.baidu.com/s/1tqB-GMvuiGQBvEl-yZ-oBw?pwd=6666)<br/>
[F] [minillm-v1(0.1B)](https://pan.baidu.com/s/1p713loS7EfwHQf3G9eYI3Q?pwd=6666)<br/>
...
* MiniLLM series ranking very intuitive, larger parameters + sufficient training data score higher. Hallucinations and errors obviously better than small models.
...
MiniLLM supports RoPE position encoding length extrapolation through YaRN algorithm, enabling models to handle text sequences exceeding training length.
...
Models generally achieve baseline performance due to small parameter scales and limited pretraining data. MiniLLM without targeted leaderboard optimization provides fair reference results.
...
## OpenAI-API Based MiniLLM Service Interface
...
* Download model weights from [Huggingface](https://huggingface.co/collections/jingyaogong/minillm-66caf8d999f5c7fa64f399e5), file structure:
    ```
    <MiniLLM-Model-Name> (root dir)
    ├─<MiniLLM-Model-Name>
    |  ├── config.json
...
```bash
vllm serve ./MiniLLM2 --model-impl transformers --served-model-name "minillm" --port 8998
```
...
```text
# Add MiniLLM2 tokenizer support
if res is None:
    res = "smollm"
```

Convert custom-trained minillm model -> gguf

```bash
python convert_hf_to_gguf.py ../minillm/MiniLLM2/
```

Quantize model

```bash
./build/bin/llama-quantize ../minillm/MiniLLM2/MiniLLM2-109M-F16.gguf ../minillm/MiniLLM2/Q4-MiniLLM2.gguf Q4_K_M
```

Command line inference

```bash
./build/bin/llama-cli -m ../minillm/MiniLLM2/MiniLLM2-109M-F16.gguf --chat-template chatml
```
...
Load custom gguf model through ollama, create new minillm.modelfile:

```text
FROM ./MiniLLM2-109M-F16.gguf
...
Load model and name as `minillm2`

```bash
ollama create -f minillm.modelfile minillm2
```
...
```text
ollama run minillm2
> Hello, I am MiniLLM2, a model based on xxxxxxxx
```
