...
class ModelEvaluator:
    ...
    def _create_default_tokenizer(self):
        ...
        vocab = ["<pad>", "<unk>", "<bos>", "<eos>", "<|tool_call|>", "<|ultra_think|>"] + list(
            chars
        )
    ...
    def evaluate_generation_quality(self, prompts: list[str]) -> dict[str, Any]:
        ...
        results["avg_response_length"] = sum(results["response_lengths"]) / len(
            results["response_lengths"]
        )
        results["avg_diversity_score"] = sum(results["diversity_scores"]) / len(
            results["diversity_scores"]
        )
        results["avg_coherence_score"] = sum(results["coherence_scores"]) / len(
    ...
    def evaluate_tool_calling(self, tool_queries: list[str]) -> dict[str, Any]:
        ...
        results["tool_detection_accuracy"] = (
            correct_detections / len(tool_queries) if tool_queries else 0
        )
    ...
    def evaluate_ultra_think(self, thinking_problems: list[str]) -> dict[str, Any]:
        ...
        for problem in thinking_problems:
            prompt = (
                f"作为alex-ckl.com的AI助手，请展示ultra think能力分析：{problem}\n\n<ultra_think>"
            )
        ...
        if results["thinking_depth_scores"]:
            results["avg_thinking_depth"] = sum(results["thinking_depth_scores"]) / len(
                results["thinking_depth_scores"]
            )
        if results["analysis_quality_scores"]:
            results["avg_analysis_quality"] = sum(results["analysis_quality_scores"]) / len(
                results["analysis_quality_scores"]
            )
...
def load_test_data(data_dir: str) -> dict[str, list[str]]:
    """加载测试数据"""
    test_data = {"general_text": [], "tool_queries": [], "ultra_think_problems": []}
