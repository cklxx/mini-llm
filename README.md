...
[🔗🍓推理模型](https://www.modelscope.cn/studios/gongjy/MiniLLM-Reasoning) · [🔗🤖常规模型](https://www.modelscope.cn/studios/gongjy/MiniLLM) · [🔗🎞️视频介绍](https://www.bilibili.com/video/BV12dHPeqE72/?share_source=copy_web&vd_source=670c2504f88726f8cf4a21ef6147c0e8)
...
> [!NOTE]
> （截至2025-10）MiniLLM系列已完成多个型号模型的预训练，最小仅需25.8M（0.02B），即可具备流畅对话能力！
...
- MiniLLM-LLM结构的全部代码（Dense+MoE模型）。
...
- 复现(蒸馏/RL)大型推理模型DeepSeek-R1的MiniLLM-Reason模型，**数据+模型**全部开源！
...
- 重要更新
- 如有兼容性需要，可访问[🔗旧仓库内容🔗](https://github.com/jingyaogong/minillm/tree/7da201a944a90ed49daef8a0265c959288dff83a)。
- MiniLLM模型参数完全改名，对齐Transformers库模型（统一命名）。
...
由于Llama位置编码方式与minillm存在区别，导致映射Llama模型后QK值存在差异
MiniLLM2系列旧模型均经过权重映射+（微调训练）QKVO线性层校准恢复而来。
本次更新后将放弃对`minillm-v1`全系列的维护，并在仓库中下线。
```
...
- 迎来发布以来重大更新，Release MiniLLM2 Series。
- 代码几乎全部重构，使用更简洁明了的统一结构。
  如有旧代码的兼容性需要，可访问[🔗旧仓库内容🔗](https://github.com/jingyaogong/minillm/tree/6e9cd28ef9b34a0a10afbdf6f59e65cb6e628efb)。
- 免去数据预处理步骤。统一数据集格式，更换为`jsonl`格式杜绝数据集下载混乱的问题。
- MiniLLM2系列效果相比MiniLLM-V1显著提升。
...
- MiniLLM2-DeepSeek-R1系列蒸馏模型诞生！
- MiniLLM2具备一定的英文能力！
- 更新MiniLLM2与第三方模型的基于更多大模型榜单测试性能的结果。
...
- 为MiniLLM拓展了多模态能力之---视觉
- 移步孪生项目[minillm-v](https://github.com/jingyaogong/minillm-v)查看详情！
...
- 更新minillm-v1-moe模型
- 为了防止歧义，不再使用mistral_tokenizer分词，全部采用自定义的minillm_tokenizer作为分词器。
...
- 更新minillm-v1 (108M)模型，采用minillm_tokenizer，预训练轮次3 + SFT轮次10，更充分训练，性能更强。
- 项目已部署至ModelScope创空间，可以在此网站上体验：
- [🔗ModelScope在线体验🔗](https://www.modelscope.cn/studios/gongjy/minillm)
...
从下文提供的[数据集下载链接](https://www.modelscope.cn/datasets/gongjy/minillm_dataset/files)
...
也可以直接去[此处](https://www.modelscope.cn/models/gongjy/MiniLLM2-PyTorch/files)下载使用我训练的`*.pth`文件。
...
【注】：25年6月后，国内网络环境无法直连WandB，MiniLLM项目默认转为使用[SwanLab](https://swanlab.cn/)作为训练可视化工具（完全兼容WandB API），即`import wandb`改为`import swanlab as wandb`即可，其他均无需改动。
...
可以选择自己构造词表训练一个“词典”，代码可见`./scripts/train_tokenizer.py`（仅供学习参考，若非必要无需再自行训练，MiniLLM已自带tokenizer）。
或者选择比较出名的开源大模型分词器，
...
因为MiniLLM体积需要严格控制，为了避免模型头重脚轻（词嵌入embedding层参数在LLM占比太高），所以词表长度短短益善。
...
<table>
  ...
  <tr><td>minillm tokenizer</td><td>6,400</td><td>自定义</td></tr>
</table>

> 👉2024-09-17更新：为了防止过去的版本歧义&控制体积，minillm所有模型均使用minillm_tokenizer分词，废弃所有mistral_tokenizer版本。

```
# 一些自言自语
> 尽管minillm_tokenizer长度很小，编解码效率弱于qwen2、glm等中文友好型分词器。
> 但minillm模型选择了自己训练的minillm_tokenizer作为分词器，以保持整体参数轻量，避免编码层和计算层占比失衡，头重脚轻，因为minillm的词表大小只有6400。
> 且minillm在实际测试中没有出现过生僻词汇解码失败的情况，效果良好。
...
经历了MiniLLM-V1的低质量预训练数据，导致模型胡言乱语的教训，`2025-02-05` 之后决定不再采用大规模无监督的数据集做预训练。
...
如果有时间+效果work（但99%基模能力不足）我会在之后更新MiniLLM基于RL训练的推理模型而不是蒸馏模型。
...
> [!NOTE]
> 2025-02-05后，开源MiniLLM最终训练所用的所有数据集，因此无需再自行预处理大规模数据集，避免重复性的数据处理工作。

MiniLLM训练数据集下载地址： [ModelScope](https://www.modelscope.cn/datasets/gongjy/minillm_dataset/files) | [HuggingFace](https://huggingface.co/datasets/jingyaogong/minillm_dataset/tree/main)
...
* `dpo.jsonl` --RLHF阶段数据集
* `lora_identity.jsonl` --自我认知数据集（例如：你是谁？我是minillm...），推荐用于lora训练（亦可用于全参SFT，勿被名字局限）
...
* MiniLLM2 Series均经过共约20GB语料训练，大约4B tokens，即对应上面的数据组合训练结果（开销：💰💰💰💰💰💰💰💰，效果：😊😊😊😊😊😊）

* 想要最快速度从0实现Zero模型，推荐使用`pretrain_hq.jsonl` + `sft_mini_512.jsonl` 的数据组合，具体花销和效果可查看下文表格（开销：💰，效果：😊😊）

* 推荐具备一定算力资源或更在意效果的朋友可以考虑前者完整复现MiniLLM2；仅有单卡GPU或在乎短时间快速复现的朋友强烈推荐后者；
...
MiniLLM-Dense（和[Llama3.1](https://ai.meta.com/blog/meta-llama-3-1/)一样）使用了Transformer的Decoder-Only结构，跟GPT-3的区别在于：
...
MiniLLM-MoE模型，它的结构基于Llama3和[Deepseek-V2/3](https://arxiv.org/pdf/2405.04434)中的MixFFN混合专家模块。

* DeepSeek-V2在前馈网络（FFN）方面，采用了更细粒度的专家分割和共享的专家隔离技术，以提高Experts的效果。

---

MiniLLM的整体结构一致，只是在RoPE计算、推理函数和FFN层的代码上做了一些小调整。
其结构如下图（重绘版）：

修改模型配置见[./model/model_minillm.py](./model/model_minillm.py)。
参考模型参数版本见下表：
...
这与MiniLLM在训练过程中，模型参数量在`d_model`和`n_layers`之间进行调整实验观察到的效果是一致的。
...
* 因此MiniLLM设定small模型dim=512，n_layers=8来获取的「极小体积<->更好效果」的平衡。
...
<br/>即可从0训练出MiniLLM-Zero-0.025B模型!!!

> MiniLLM2-Small参数
>> `pretrain_hq`+`sft_512`+`sft_2048`+`dpo`数据集
<br/>单卡3090 (2 epochs) + 大约38.16小时 + 花费49.61元人民币
<br/>即可从0训练出MiniLLM2-Small-0.025B模型!!!

> MiniLLM2参数
>> `pretrain_hq`+`sft_512`+`sft_2048`+`dpo`数据集
<br/>单卡3090 (2 epochs) + 大约122小时 + 花费158.6元人民币
<br/>即可从0训练出MiniLLM2-0.1B模型!!!

</details>



✨基于单卡NVIDIA 3090的`MiniLLM-Zero`从0训练仅需`2小时` + `3块钱`，实现ChatBot效果！

✨PS：若采用8卡4090训练，总用时甚至可以压缩到10分钟以内！（由于时间更短，花费同样3元左右，与单卡成本相当）

✨以极低极低的门槛，实现人人可玩的大模型自由！这正是MiniLLM系列的诞生初衷！

✨仅价值`3块钱`成本的`MiniLLM-Zero`并不是噱头！Chat测试：
...
Zero模型权重保存为 `full_sft_512_zero.pth`（见下文MiniLLM模型文件链接），如有兴趣可下载检验此模型效果。
...
在训练时，MiniLLM的指令和回答长度被截断在512，是为了节省显存空间。就像学习写作时，会先从短的文章开始，当学会写作200字作文后，800字文章也可以手到擒来。
...
此处应当着重介绍MiniLLM实现的白盒蒸馏代码`train_distillation.py`，由于MiniLLM同系列本身并不存在强大的教师模型，因此白盒蒸馏代码仅作为学习参考。
...
```
 {"conversations": [{"role": "user", "content": "你叫什么名字？"}, {"role": "assistant", "content": "我叫minillm..."}]}
...
👶: 你和openai是什么关系？
🤖️: 我是 MiniLLM，一个由 Jingyao Gong 开发的人工智能助手。我通过自然语言处理和算法训练来与用户进行交互。
```
...
<del>
MiniLLM2第一时间只能坚定不移的选择做蒸馏派，日后基于0.1B模型的RL如果同样取得小小进展会更新此部分的训练方案。
</del>
...
```json lines
{
  "conversations": [
    ...
    {
      "role": "assistant",
      "content": "<think>\n你好！我是由中国的个人开发者独立开发的智能助手MiniLLM-R1-Lite-Preview，很高兴为您提供服务！\n</think>\n<answer>\n你好！我是由中国的个人开发者独立开发的智能助手MiniLLM-R1-Lite-Preview，很高兴为您提供服务！\n</answer>"
    }
  ]
}
```
...
👶: 你更擅长哪一个学科？
🤖️: <think>
您好！我是由中国的个人开发者开发的智能助手MiniLLM-R1。如您有任何任何问题，我会尽我所能为您提供帮助。
</think>
<answer>
您好！我是由中国的个人开发者开发的智能助手MiniLLM-R1。如您有任何任何问题，我会尽我所能为您提供帮助。
</answer>
...
MiniLLM 着手实现**2+N**种基本+前沿的RLAIF方法：
* **PPO**、**GRPO** 被大规模验证的经典RL算法；
* N种前沿RL算法（不定期以Exp性质更新）。

#### 1️⃣ 数据集准备 (需要)

为了快速验证RLAIF的效果，这里从SFT数据集中随机采样了1万条高质量对话，构建约1MB大小的`rlaif-mini.jsonl`([Huggingface](https://huggingface.co/datasets/jingyaogong/minillm_dataset/blob/main/rlaif-mini.jsonl))
...
下载奖励模型后需要放置在minillm项目的**同级目录**下，推荐结构如下：
...
│   ├── model/
│   └── ...
└── internlm2-1_8b-reward/       # 奖励模型（与minillm同级）
    ├── config.json
    ├── model.safetensors
    └── ...
```
...
<details>
<summary><b>奖励机制选择与MiniLLM限制说明（点击展开）</b></summary>
...
然而对于MiniLLM这种0.1B参数量极小能力弱的模型，在通用任务（如R1风格的数学数据集）上会遇到严重的奖励稀疏(Reward Sparsity)问题：
...
为缓解此问题，MiniLLM的实现选择了**model-based的连续性奖励信号**：
...
- minillm实践中避免直接使用rule-based二元奖励 + 超纲难度数据（如MATH500），易导致奖励全零；
...
MiniLLM的PPO实现包含了Actor模型(生成回答)和Critic模型(评估回答价值)，以及完整的GAE(Generalized Advantage Estimation)优势函数计算。
...
在MiniLLM这种超小模型上，这个问题尤为明显，求解数学问题99.99%的情况下整组回答质量都很差，那么将无法学习。
...
> 注：SPO是实验性前沿算法，MiniLLM的实现用于探索学习。由于模型参数量极小，无法完全复现论文的8B模型效果。
...
> 考虑到多人反应百度网盘速度慢，MiniLLM2及以后全部使用ModelScope/HuggingFace托管。

#### ① PyTorch原生模型

MiniLLM2模型权重 ([ModelScope](https://www.modelscope.cn/models/gongjy/MiniLLM2-PyTorch) | [HuggingFace](https://huggingface.co/jingyaogong/MiniLLM2-Pytorch))
...
MiniLLM系列 ([ModelScope](https://www.modelscope.cn/collections/MiniLLM-b72f4cfeb74b47)
...
测试基于`full_sft`和`rlhf`的`MiniLLM2系列`模型对比，测试随机种子均固定
...
DPO和在线PPO的区别在于reject和chosen都是离线准备的，和minillm模型本身的输出必然存在很大的分布差异。
通俗地说DPO算法使模型观看乒乓球世界冠军的打法「录像」进行RL，而不是像PPO一样请reward模型做「教练」纠正自己的打法进行RL。

## Ⅱ RLAIF对比

基于`MiniLLM2 (768dim)`在相同数据与训练步数下的主观观感对比，测试随机种子均固定2026，供参考：

[A] MiniLLM-Reason-0.1B (蒸馏)<br/>
...
* MiniLLM系列的排序非常符合直觉，参数越大+训练数据越充分评分越高，幻觉和错误都会比小模型肉眼可见的好。
...
MiniLLM支持通过YaRN算法进行RoPE位置编码的长度外推，使模型能够处理超出训练长度的文本序列。
...
MiniLLM模型本身预训练数据集小的可怜，也没有针对性的对测试集做刷榜微调，因此结果纯娱乐：

| models                                                                        | from          | params↓ | ceval↑ | cmmlu↑ | aclue↑ | tmmlu+↑ |
|-------------------------------------------------------------------------------|---------------|---------|--------|---------|--------|---------|
| MiniLLM2                                                                     | JingyaoGong   | 104M    | 26.52  | 24.42   | 24.97  | 25.27   |
| MiniLLM2-Small                                                               | JingyaoGong   | 26M     | 26.37  | 24.97   | 25.39  | 24.63   |
| MiniLLM2-MoE                                                                 | JingyaoGong   | 145M    | 26.6   | 25.01   | 24.83  | 25.01   |
...
* 从[Huggingface](https://huggingface.co/collections/jingyaogong/minillm-66caf8d999f5c7fa64f399e5)下载模型权重文件，文件树：
    ```
...
```text
# 添加MiniLLM2 tokenizer支持
if res is None:
    res = "smollm"
```

转换自定义训练的minillm模型 -> gguf
...
通过ollama加载自定义的gguf模型，新建minillm.modelfile：
